...
export function MainScreen({ navigation }: MainScreenProps) {
  ...
  const generarOutfit = async () => {
    try {
      const token = await AsyncStorageService.getToken();
      const response = await fetch(`${API_URL}/outfit/generate`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ clima: climaSeleccionado }),
      });
      const data = await response.json();
      if (!response.ok) {
    ...
    if (!data.superior || !data.inferior) {
      Alert.alert(
        'Faltan prendas',
        'Para este tipo de clima necesitas más prendas registradas.',
        [
      ...
      Alert.alert('Error al generar outfit', 'Por favor, inténtalo de nuevo.');
  ...
  return (
    <ScrollView style={{ flex: 1 }}>
      <View style={mainStyles.container}>
        ...
        <TouchableOpacity style={mainStyles.button} onPress={() => navigation.navigate('AddPrenda')}>
          <Text style={mainStyles.buttonText}>➕ Agregar nueva prenda</Text>
        </TouchableOpacity>

        <TouchableOpacity style={mainStyles.button} onPress={() => navigation.navigate('ManagePrendas')}>
